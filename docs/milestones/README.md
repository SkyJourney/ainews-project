...
flowchart LR
    ...
    M5 --> M6["M6 前端上线（已完成）"]
    M6 --> M7["M7 生产化收尾（开发完成，观察期中）"]
    M6 -.提前完成.-> M8["M8 旧数据迁移（已完成）"]
    M7 -.延后.-> M9["M9 向量化/RAG（可选延伸）"]
    M7 -.数据门槛达成.-> M10["M10 Deep Dive（跨天聚合，已完成）"]
    M10 -.正交扩展.-> M11["M11 Topic Deep Dive（专题月报，进行中）"]
```
...
| M6 | [M6-frontend.md](./M6-frontend.md) | 前端上线（Astro SSR 接入 Postgres） | **已完成**（2026-07-05） |
| M7 | [M7-production-hardening.md](./M7-production-hardening.md) | 生产化收尾，可独立连续运行 | **开发完成（2026-07-05），7 天观察期中** |
| M8 | [M8-legacy-migration.md](./M8-legacy-migration.md) | 旧 vault 历史数据迁移 | **已完成**（2026-07-05，提前于 M7 观察期结束执行，用户明确要求） |
| M9 | [M9-vectorization.md](./M9-vectorization.md) | 向量化 / RAG 扩展（可选延伸） | 延后 |
| M10 | [M10-deep-dive.md](./M10-deep-dive.md) | Deep Dive：跨天聚合深度解读（旧系统从未实现，净新设计） | **已完成**（2026-07-09） |
| M11 | [M11-topic-deep-dive.md](./M11-topic-deep-dive.md) | Topic Deep Dive：专题月报（M10 正交扩展，净新设计） | **设计已确认，实现进行中**（2026-07-09） |
